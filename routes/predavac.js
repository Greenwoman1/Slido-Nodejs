...
router.post('/zakazi_predavanje', upload.single('pozadina'), async function (req, res) {
  ...
  var predavanjee = new predavanje({
    ...
    repeat_days: req.body.repeatDays,
    pozadina: filename
...
;
    client.on('odgovor_pitanja', async function(data){
      var i = data.idd;
      var o = data.o;
      console.log(o);
       predavanje.updateOne({kod: req.params.kod}, {$inc: {broj_odgovorenih: 1}}, {new: true}, (error, result) => {
        if (error) {
          console.log(error);
        } else {
          console.log(result);
       ...
       pitanja.findByIdAndUpdate(i, {odgovor: o},{new: true}, (error, doc) => {
        if (error) {
          ...
          client.emit('odgovori_na_pitanje', doc);
    ...
    client.on('posalji_email', async function (niz_emailova){
      ...
      var mailOptions = {
        ...
        html: '<h1>Dobro došli</h1><p>Pristupite predavanju koristeći link: http://localhost:3000/gost/'+ req.params.kod+'</p>'
      };
